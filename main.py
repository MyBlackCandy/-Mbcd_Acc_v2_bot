...
async def handle_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.reply_to_message:
        target_user = get_user_name(update.message.reply_to_message.from_user)
        reply_msg_id = update.message.reply_to_message.message_id
    else:
        target_user = get_user_name(update.message.from_user)
        reply_msg_id = None
...
async def report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, r in enumerate(reversed(rows), 1):
        ...
        if r[3] and chat_id.startswith("-100"):
            link = f"https://t.me/c/{chat_id[4:]}/{r[3]}"
        else:
            link = "-"
